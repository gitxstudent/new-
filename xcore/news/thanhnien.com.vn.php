<?php
function thanhnien_com_vn($url){
		$data = xcurl($url,$url,"","","FF");
		$html = str_get_html($data);
		foreach($html->find('img') as $e) $e->src = 'http://www.thanhnien.com.vn'.$e->src;
		...
		$xata['title'] = strip_tags($html->find('.mainTitle', 0)->innertext);
		$xata['data'] = $html->find('.pageContent', 0)->innertext;
		return $xata;
	}