<?php
function infotv_vn($url){
		$data = xcurl($url,$url,"","","FF");
		$html = str_get_html($data);
		foreach($html->find('img') as $e) $e->src = 'http://www.infotv.vn'.$e->src;
		...
		foreach($html->find('.related-news,.advs') as $e) $e->outertext = '';
		$xata['title'] = strip_tags($html->find('a[class=contentpagetitle]', 0)->innertext);
		$xata['data'] = $html->find('div[class=article-content]', 0)->innertext;
		return $xata;
	}